//! Claude Code integration for dapz.
//!
//! Provides `dapz init` to register the MCP server and inject context files.
...
use self::awareness::{
    claude_md_has_ref, dapz_md_exists, patch_claude_md_ref, remove_claude_md_ref, remove_dapz_md,
    write_dapz_md,
};
use self::settings::{
...
fn run_init(
    ...
    if !cfg!(feature = "mcp") {
        eprintln!(
            "Error: This binary was built without `mcp` feature.\n  \
             `dapz init` requires MCP support. Rebuild with `--features mcp`."
        );
    ...
    // 1. Write DAPZ.md
    match write_dapz_md(global, dry_run, force) {
        Ok(true) => println!("{prefix}Wrote DAPZ.md"),
        Ok(false) => println!("DAPZ.md already up to date"),
        Err(e) => {
            eprintln!("Error writing DAPZ.md: {e}");
            return ExitCode::FAILURE;
        }
    }

    // 2. Patch CLAUDE.md with @DAPZ.md reference
    match patch_claude_md_ref(global, dry_run, force) {
        Ok(true) => println!("{prefix}Added @DAPZ.md to CLAUDE.md"),
        Ok(false) => println!("CLAUDE.md already contains @DAPZ.md"),
        Err(e) => {
...
fn run_uninstall(global: bool, dry_run: bool) -> ExitCode {
    ...
    // 1. Remove DAPZ.md
    match remove_dapz_md(global, dry_run) {
        Ok(true) => println!("{prefix}Removed DAPZ.md"),
        Ok(false) => println!("DAPZ.md not found"),
        Err(e) => {
            eprintln!("Error removing DAPZ.md: {e}");
            return ExitCode::FAILURE;
        }
    }

    // 2. Remove @DAPZ.md reference from CLAUDE.md
    match remove_claude_md_ref(global, dry_run) {
        Ok(true) => println!("{prefix}Removed @DAPZ.md from CLAUDE.md"),
        Ok(false) => println!("@DAPZ.md reference not found in CLAUDE.md"),
        Err(e) => {
            eprintln!("Error patching CLAUDE.md: {e}");
            return ExitCode::FAILURE;
        }
...
fn show_config(global: bool) -> ExitCode {
    ...
    let dapz_ok = dapz_md_exists(global);
    let ref_ok = claude_md_has_ref(global);

    println!("dapz Configuration ({scope}):\n");
    println!("  Binary:    {}", binary_path.display());
    println!("  MCP:       {} {}", status_icon(mcp_ok), sp.display());
    println!("  DAPZ.md:   {}", status_icon(dapz_ok));
...
fn format_mcp_entry(binary_path: &std::path::Path) -> String {
    format!(
        r#"{{"mcpServers": {{"dapz": {{"command": "{}", "args": ["mcp"]}}}}}}"#,
        binary_path.display()
    )
}
