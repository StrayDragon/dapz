...
///
/// Checks `DAPZ_CLAUDE_DIR` env var first, then falls back to `~/.claude/`.
pub fn resolve_claude_dir() -> Result<PathBuf, DapzError> {
...
pub fn resolve_binary_path() -> Result<PathBuf, DapzError> {
    std::env::current_exe()
        .map_err(|e| DapzError::Config(format!("Cannot determine binary path: {e}")))
}

/// Atomic write: write to a tempfile in the same directory, then rename.
pub fn atomic_write(path: &Path, content: &[u8]) -> Result<(), DapzError> {
    let dir = path
        .parent()
        .ok_or_else(|| DapzError::Config(format!("No parent directory for {}", path.display())))?;

    let mut tmp = tempfile::NamedTempFile::new_in(dir)
        .map_err(|e| DapzError::Config(format!("Failed to create tempfile: {e}")))?;

    tmp.write_all(content)
        .map_err(|e| DapzError::Config(format!("Failed to write tempfile: {e}")))?;

    tmp.persist(path)
        .map_err(|e| DapzError::Config(format!("Failed to persist tempfile: {e}")))?;

    Ok(())
}

/// Read settings.json, or return an empty object if the file doesn't exist.
fn read_settings(path: &Path) -> Result<Value, DapzError> {
    if !path.exists() {
    ...
    serde_json::from_str(&content)
        .map_err(|e| DapzError::Config(format!("Invalid JSON in {}: {e}", path.display())))
}

/// Check if the dapz MCP server entry is already present with the correct binary path.
...
/// Patch settings.json with the dapz MCP server entry.
///
...
pub fn patch_mcp_server(
    ...
    let mcp_servers = root
        .as_object_mut()
        .ok_or_else(|| DapzError::Config("settings.json is not an object".into()))?
        .entry("mcpServers")
        .or_insert_with(|| serde_json::json!({}));

    mcp_servers
        .as_object_mut()
        .ok_or_else(|| DapzError::Config("mcpServers is not an object".into()))?
        .insert(MCP_SERVER_KEY.to_string(), mcp_entry);

    let content = serde_json::to_string_pretty(&root)
        .map_err(|e| DapzError::Config(format!("Failed to serialize settings: {e}")))?;

    // Backup original if it exists
    if settings_path.exists() {
        let backup = settings_path.with_extension("json.bak");
        fs::copy(settings_path, &backup)
            .map_err(|e| DapzError::Config(format!("Failed to backup settings: {e}")))?;
    }
...
/// Remove the dapz MCP server entry from settings.json.
///
/// Returns `true` if an entry was removed, `false` if none existed.
pub fn remove_mcp_server(settings_path: &Path, dry_run: bool) -> Result<bool, DapzError> {
    let mut root = read_settings(settings_path)?;
...
/// Check if the dapz MCP server is registered in settings.json.
...
pub fn settings_path(global: bool) -> Result<PathBuf, DapzError> {
    if global {
...
mod tests {
    ...
    fn patch_mcp_server_creates_entry() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("settings.json");
        let binary = PathBuf::from("/usr/bin/dapz");
        ...
        assert_eq!(settings["mcpServers"]["dapz"]["command"], "/usr/bin/dapz");
        assert_eq!(
            settings["mcpServers"]["dapz"]["args"],
            serde_json::json!(["mcp"])
        );
    }
    ...
    fn remove_mcp_server_removes_entry() {
        ...
        fs::write(
            &path,
            r#"{"mcpServers": {"dapz": {"command": "/usr/bin/dapz", "args": ["mcp"]}, "other": {"command": "/usr/bin/other"}}}"#,
        )
        .unwrap();
        ...
        let content = fs::read_to_string(&path).unwrap();
        let settings: Value = serde_json::from_str(&content).unwrap();
        assert!(settings["mcpServers"].get("dapz").is_none());
        assert_eq!(settings["mcpServers"]["other"]["command"], "/usr/bin/other");
    }
    ...
    fn is_mcp_registered_true() {
        ...
        fs::write(
            &path,
            r#"{"mcpServers": {"dapz": {"command": "/usr/bin/dapz"}}}"#,
        )
        .unwrap();
    ...
    fn is_mcp_registered_wrong_path() {
        ...
        fs::write(
            &path,
            r#"{"mcpServers": {"dapz": {"command": "/other/path/dapz"}}}"#,
        )
        .unwrap();
