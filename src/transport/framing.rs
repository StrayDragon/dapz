//! Shared Content-Length framing for DAP transports.
//!
//! DAP uses the same HTTP-like header `Content-Length: N\r\n\r\n` followed by N
//! bytes of JSON body as LSP. Ported from lspz's cancel-safe frame reader.

use tokio::io::{AsyncRead, AsyncReadExt, BufReader};

use crate::error::DapzError;

/// Maximum DAP message body size: 16 MiB.
...
pub(crate) async fn read_frame_with_state<R: AsyncRead + Unpin>(
    ...
    while state.content_length.is_none() {
        ...
        if state.header.len() > MAX_HEADER_SIZE {
            state.clear();
            return Err(DapzError::Protocol("header too large".into()));
        }

        if state.header.len() >= 4 && state.header[state.header.len() - 4..] == *b"\r\n\r\n" {
            let header = std::str::from_utf8(&state.header)
                .map_err(|_| DapzError::Protocol("header is not valid UTF-8".into()))?;
...
pub(crate) fn parse_content_length(header: &str) -> Result<u64, DapzError> {
...
mod tests {
    ...
    fn test_missing_content_length() {
        ...
        match parse_content_length(header) {
            Err(DapzError::Protocol(msg)) => assert!(msg.contains("missing")),
    ...
    fn test_invalid_content_length() {
        ...
        match parse_content_length(header) {
            Err(DapzError::Protocol(msg)) => assert!(msg.contains("invalid")),
            _ => panic!("expected Protocol error"),
        }
    }
    ...
    async fn test_read_frame_truncated_body() {
        ...
        assert!(matches!(result, Err(DapzError::ServerExited)));
    }
    ...
    async fn test_read_frame_rejects_oversized_body() {
        ...
        let result = read_frame_with_state(&mut reader, &mut state).await;
        match result {
            Err(DapzError::Protocol(msg)) => assert!(msg.contains("body too large")),
    ...
    async fn test_frame_state_survives_partial_header() {
        ...
        assert!(matches!(r1, Err(DapzError::ServerExited)));
